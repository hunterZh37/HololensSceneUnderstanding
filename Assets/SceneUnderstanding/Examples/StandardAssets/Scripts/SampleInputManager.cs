namespace Microsoft.MixedReality.SceneUnderstanding.Samples.Unity
{
    ...
    public class SampleInputManager : MonoBehaviour
    {
        ...
        private void AddDefaultCommands()
        {
            ...
            inputActions.Add(InputAction.Create("Toggle Ceilings", KeyCode.Alpha5, "Enable / Disable ceilings surfaces", async () =>
            {
                SuManager.FilterCeilingSceneObjects = !SuManager.FilterCeilingSceneObjects;
                await SuManager.DisplayDataAsync();
            }));
        ...
        private void MiniMapOn()
        {
            if (suMinimap == null)
            {
                ...
                string quadSaveData = "";
                foreach (Transform child in suMinimap.transform) {
                    if (child.gameObject.name != "Ceiling") {
                        QuadData quad = new QuadData(child.gameObject.name, child.gameObject.transform.position, child.gameObject.transform.rotation, child.gameObject.transform.localScale);
                        quadSaveData += JsonUtility.ToJson(quad);
                        File.WriteAllText(Application.dataPath + "/JSON/quadData.json", quadSaveData.ToString());
                        //StreamWriter writer = new StreamWriter(Application.dataPath + "/JSON/quadData.json", true);
                        //writer.WriteLine(quadSaveData.ToString());
                    }
                }
               
                //string quadSaveData = JsonUtility.ToJson(quad);
               
                //var file = File.Create(Application.dataPath + "/JSON/quadData.json"); // returns a FileInfo object
                //File.WriteAllText(Application.dataPath + "/JSON/quadData.json", quadSaveData.ToString());
